...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def chat_message(self, event):
        ...
        sender = await self.get_user_profile_by_id(sender_id)
        print(sender)
        sender_pic = sender.profile_picture.url

        # Send message to WebSocket
        await self.send(text_data=json.dumps({"message": message, "sender_id": sender_id, "receiver_id": receiver_id, "sender_pic": sender_pic}))
